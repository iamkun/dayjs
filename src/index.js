...
class Dayjs {
  ...
  format(formatStr = 'YYYY-MM-DDTHH:mm:ssZ') {
    ...
    return formatStr.replace(/Y{2,4}|Q|M{1,4}|Do|D{1,2}|d{1,4}|X|x|H{1,2}|h{1,2}|k{1,2}|m{1,2}|s{1,2}|Z{1,2}/g, (match) => {
      switch (match) {
        case 'YY':
        ...
        case 'h':
          ...
          return this.$H < 12 ? this.$H : this.$H - 12
        case 'hh':
          if (this.$H === 0) {
            return 12
          }
          return Utils.padStart(String(this.$H < 12 ? this.$H : this.$H - 12), 2, '0')
        case 'k':
