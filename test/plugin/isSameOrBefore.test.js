...
test('is same or before without units', () => {
  ...
  const mCopy = dayjs(m)
  expect(m.isSameOrBefore(dayjs(new Date(2012, 3, 2, 3, 5, 5, 10)))).toBe(
    true,
    'year is later'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2010, 3, 2, 3, 3, 5, 10)))).toBe(
    false,
    'year is earlier'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 4, 2, 3, 4, 5, 10)))).toBe(
    true,
    'month is later'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 2, 2, 3, 4, 5, 10)))).toBe(
    false,
    'month is earlier'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 3, 3, 4, 5, 10)))).toBe(
    true,
    'day is later'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 1, 3, 4, 5, 10)))).toBe(
    false,
    'day is earlier'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 4, 4, 5, 10)))).toBe(
    true,
    'hour is later'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 2, 4, 5, 10)))).toBe(
    false,
    'hour is earlier'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 3, 5, 5, 10)))).toBe(
    true,
    'minute is later'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 3, 3, 5, 10)))).toBe(
    false,
    'minute is earlier'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 3, 4, 6, 10)))).toBe(
    true,
    'second is later'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 3, 4, 4, 11)))).toBe(
    false,
    'second is earlier'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 3, 4, 5, 10)))).toBe(
    true,
    'millisecond match'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 3, 4, 5, 11)))).toBe(
    true,
    'millisecond is later'
  )
  expect(m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 3, 4, 5, 9)))).toBe(
    false,
...
test('is same or before day', () => {
  ...
  expect(m.isSameOrBefore(dayjs(new Date(2011, 1, 2, 0, 0, 0, 0)), 'day')).toBe(
    true,
    'exact start of day'
  )
  expect(
  ...
  expect(m.isSameOrBefore(dayjs(new Date(2011, 1, 3, 0, 0, 0, 0)), 'day')).toBe(
    true,
    'start of next day'
  )
  expect(
    m.isSameOrBefore(dayjs(new Date(2011, 1, 1, 23, 59, 59, 999)), 'day')
  ).toBe(false, 'end of previous day')
  expect(m.isSameOrBefore(m, 'day')).toBe(
    true,
...
test('is same or before hour', () => {
  ...
  ).toBe(true, 'start of next hour')
  expect(
    m.isSameOrBefore(dayjs(new Date(2011, 1, 2, 2, 59, 59, 999)), 'hour')
  ).toBe(false, 'end of previous hour')
  expect(m.isSameOrBefore(m, 'hour')).toBe(
    true,
...
test('is same or before minute', () => {
  ...
  ).toBe(true, 'start of next minute')
  expect(
    m.isSameOrBefore(dayjs(new Date(2011, 1, 2, 3, 3, 59, 999)), 'minute')
  ).toBe(false, 'end of previous minute')
  expect(m.isSameOrBefore(m, 'minute')).toBe(
    true,
...
test('is same or before second', () => {
  ...
  ).toBe(true, 'start of next second')
  expect(
    m.isSameOrBefore(dayjs(new Date(2011, 1, 2, 3, 4, 4, 999)), 'second')
  ).toBe(false, 'end of previous second')
  expect(m.isSameOrBefore(m, 'second')).toBe(
    true,
...
test('is same or before millisecond', () => {
  ...
  expect(
    m.isSameOrBefore(dayjs(new Date(2011, 3, 2, 3, 4, 5, 10)), 'milliseconds')
  ).toBe(true, 'plural should work')
  expect(
  ...
  expect(m.isSameOrBefore(m, 'millisecond')).toBe(
    true,
    'same moments are in the same millisecond'
  )
  expect(+m).toEqual(
    +mCopy,
    'isSameOrBefore millisecond should not change moment'
  )
})
...
test('is same with invalid moments', () => {
  ...
  expect(invalid.isSameOrBefore(invalid)).toBe(
    false,
    'invalid moments are not considered equal'
  )
  expect(m.isSameOrBefore(invalid)).toBe(
    false,
    'valid moment is not before invalid moment'
  )
  expect(invalid.isSameOrBefore(m)).toBe(
    false,
  ...
  expect(m.isSameOrBefore(invalid, 'minute')).toBe(
    false,
    'invalid moment minute'
  )
  expect(m.isSameOrBefore(invalid, 'second')).toBe(
    false,
    'invalid moment second'
  )
  expect(m.isSameOrBefore(invalid, 'milliseconds')).toBe(
    false,
