...
const localeCustomizations = {
  ...
  preparse(string) {
    if (typeof string !== 'string') {
      throw new TypeError(`preparse - Expected string, got ${typeof string}`)
    }
    try {
      return string.replace(/[!#$%&()*@^]/g, (match) => numberMap[match])
    } catch (error) {
      throw new Error(
        `Unexpected error during preparse of '${string}' - ${error}`
      )
    }
  },
  postformat(string) {
    if (typeof string !== 'string') {
      throw new TypeError(`postformat - Expected string, got ${typeof string}`)
    }
    try {
      return string.replace(/\d/g, (match) => symbolMap[match])
    } catch (error) {
      throw new Error(
        `Unexpected error during postFormat of '${string}' - ${error}`
      )
    }
  },
}
...
describe('calendar day', () => {
  const a = dayjs().hour(12).minute(0).second(0)

  it('today at the same time', () =>
    expect(dayjs(a).calendar()).toBe('Today at !@:)) PM'))

  it('Now plus 25 min', () =>
    expect(dayjs(a).add({ m: 25 }).calendar()).toBe('Today at !@:@% PM'))

  it('Now plus 1 hour', () =>
    expect(dayjs(a).add({ h: 1 }).calendar()).toBe('Today at !:)) PM'))

  it('tomorrow at the same time', () =>
    expect(dayjs(a).add({ d: 1 }).calendar()).toBe('Tomorrow at !@:)) PM'))

  it('Now minus 1 hour', () =>
    expect(dayjs(a).subtract({ h: 1 }).calendar()).toBe('Today at !!:)) AM'))

  it('yesterday at the same time', () =>
    expect(dayjs(a).subtract({ d: 1 }).calendar()).toBe(
      'Yesterday at !@:)) PM'
    ))
})
